...
void drawRobot()
{
	...
	// Offset angle by 270 to match movement and rotation
	glRotatef(robotAngle + 270, 0.0, 1.0, 0.0);
...
void drawLeftWheel()
{
	...
	glRotatef(leftWheelAngle, 0, 0, 1);
...
void drawRightWheel()
{
	...
	glTranslatef(robotBodyWidth * -0.25, 0, robotBodyDepth * 0.45);
	glRotatef(rightWheelAngle, 0, 0, 1);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case ' ':
		...
		break;
	case 'w':
		xPos += moveSpeed * sin((robotAngle * PI) / 180);
		zPos += moveSpeed * cos((robotAngle * PI) / 180);
		leftWheelAngle -= 15;
		rightWheelAngle -= 15;
		break;
	case 's':
...
void animationHandler(int param)
{
	...
	if (!stop)
	{
		plateAngle += 1.0;
		glutPostRedisplay();
		glutTimerFunc(0, animationHandler, 10);
	}
	else
	{
		plateAngle -= 1.0;
...
void functionKeys(int key, int x, int y)
{
	...
	if (key == GLUT_KEY_F1)
	{
		...
		printf("s\t\treverse\n");
		printf("d\t\tturn right\n");
		printf("spacebar\trelease flipper. press again to rearm flipper\n");
	}
